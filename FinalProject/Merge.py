...
def getdcard(likeCount,commentCount,topics):
    print(2)
    tag=[]
    tag=comparision(topics)
    print(tag)
    print(3)
    for index,i in enumerate(tag): #index計數(0,1,2...)，i=1.jpg....
        print("我是",i,index)
        image = Image.open(f'D:/github/Python/FinalProject/image/{i}') #讀入圖片
...
btn1 = Button(window,text="搜尋",bg="white",height=2,width=5,command=getdcard(likeCount,commentCount,topics))
